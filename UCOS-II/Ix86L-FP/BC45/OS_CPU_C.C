...
*           2) OS_FP_STORAGE_SIZE  currently allocates 128 bytes of storage even though the 80486 
*              (or Pentium processors) only require 108 bytes to save the FPU context.  I decided to
*              allocate 128 bytes for future expansion.  Also, I used INT32U to ensure that storage is 
*              located on a 32-bit boundary.
*********************************************************************************************************
*/

#define  OS_NTASKS_FP         (OS_MAX_TASKS + OS_N_SYS_TASKS - 1)
#define  OS_FP_STORAGE_SIZE   (32 * sizeof(INT32U))
...
static  OS_MEM  *OSFPPartPtr;          /* Pointer to memory partion holding FPU storage areas          */
...
*              switching the Floating-Point registers.  This function is called by OSInit() at the end of 
*              OSInit().
*
* Arguments  : none
*
...
*              2) For the 80486 processor, 108 bytes are required to save the FPU context.  I decided to
*                 allocate 128 bytes (i.e. 32 * INT32U) for future expansion.  Also, I used INT32U to 
*                 ensure that storage is located on a 32-bit boundary.
...
*                 This only applies if OSTaskStat() was created with OSTaskCreateExt().
*********************************************************************************************************
*/
...
#if OS_TASK_STAT_EN && OS_TASK_CREATE_EXT_EN       /* CHANGE 'OPTIONS' for OSTaskStat()                */
...
void OSTaskSwHook (void)
{
    ...
    if (OSRunning) {                                       /* Don't save on OSStart()!                 */
